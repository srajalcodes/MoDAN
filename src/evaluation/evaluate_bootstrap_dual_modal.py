...
def get_features_for_batch(df_chunk, chem, esm, c_dim, e_dim):
    ...
    for _, row in df_chunk.iterrows():
        ...
        # DUAL-MODAL ONLY (Chem + ESM)
        vec1 = np.concatenate([c1, e1])
        vec2 = np.concatenate([c2, e2])
...
def evaluate_models(xgb_model, lr_model, csv_path, chem, esm, c_dim, e_dim, name):
    print(f"\nEvaluating {name}...")
    xgb_probs, lr_probs, all_true = [], [], []
    
    for chunk in pd.read_csv(csv_path, chunksize=10000):
        X_batch, y_batch = get_features_for_batch(chunk, chem, esm, c_dim, e_dim)
...
def main():
    ...
    c_dim, e_dim = len(next(iter(chem.values()))), len(next(iter(esm.values())))
    ...
    print("\nTraining DUAL-MODAL Baselines (Chem+ESM only)...")
    ...
    evaluate_models(xgb_model, lr_model, r"dataset\test_cold_S1.csv", chem, esm, c_dim, e_dim, "S1 (Known-New)")
    evaluate_models(xgb_model, lr_model, r"dataset\test_cold_S2.csv", chem, esm, c_dim, e_dim, "S2 (New-New)")
